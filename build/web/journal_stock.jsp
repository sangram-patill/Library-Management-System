<%-- 
    Document   : journal_stock
    Created on : 8 Apr, 2023, 10:38:08 AM
    Author     : sangr
--%>
...
<html>
    <head>
        ...
        <title>YCM Admin - Journal Stock</title> 
       ...
       <%
                  ...
                  <div class="cards">
                        <div class="card">
                            <div class="box">                                
                                ...
                                <div class="row">
                                    <div class="col-sm-6">
                                         <h4 style="margin: 0% 0% 0% 2%">Journal Stock</h4>
                                    </div>
                                    <div class="col-sm-6">
                                         <a href="journal.jsp"><button class="stock">Journal Entry</button></a>
                                    </div>
                                 </div>
                                <hr style="background-color: grey;width: 1120px;height:2px;border-width:0;margin-left: 1%">
                               
                                <div class="row">
                                    <input id="search" type="text" placeholder="Search Journal..." style="margin-left: 2.5%;">
                               </div>
                                <br>
		...
		<% 
            try {
                 ...
                 String sql = "select * from journal where status = 'Returned'"; ResultSet rs = st.executeQuery(sql);

                while (rs.next()) {
                    %>
                <form action="journal.jsp" method="post">
                        <tr>
                            <td><%=rs.getString("j_date_of_entry")%></td>
                    <input type="hidden" name="j_date_of_entry" value="<%=rs.getString("j_date_of_entry")%>">
                            <td><%=rs.getString("j_department")%></td>
                    <input type="hidden" name="j_department" value="<%=rs.getString("j_department")%>">       
                            <td><%=rs.getString("j_category")%></td>
                    <input type="hidden" name="j_category" value="<%=rs.getString("j_category")%>">
                            <td><%=rs.getString("j_acc_no")%></td>
                    <input type="hidden" name="j_acc_no" value="<%=rs.getString("j_acc_no")%>">
                            <td><%=rs.getString("j_language")%></td>
                    <input type="hidden" name="j_language" value="<%=rs.getString("j_language")%>">
                            <td><%=rs.getString("j_title")%></td>
                    <input type="hidden" name="j_title" value="<%=rs.getString("j_title")%>">
                            <td><%=rs.getString("j_subtitle")%></td>
                    <input type="hidden" name="j_subtitle" value="<%=rs.getString("j_subtitle")%>">
                            <td><%=rs.getString("j_subject")%></td>
                    <input type="hidden" name="j_subject" value="<%=rs.getString("j_subject")%>">
                            <td><%=rs.getString("j_copies")%></td>
                    <input type="hidden" name="j_copies" value="<%=rs.getString("j_copies")%>">
                            <td><%=rs.getString("j_pages")%></td>
                    <input type="hidden" name="j_pages" value="<%=rs.getString("j_pages")%>">
