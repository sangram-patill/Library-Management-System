 ...
 * sangr
 */
public class student_id extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
       ...
       if(event.equals("Download ID"))
       {
                try
                {
                    ...
                    String sql = "select * from student where registration_no= '"+session.getAttribute("username") +"'";  rs = st.executeQuery(sql);
                    
                    if(rs.next())
                    {
                        session.setAttribute("registration_no",session.getAttribute("username"));
                        resp.sendRedirect("student/stud_id_card.jsp");
                    }
                }
                catch(Exception ex)
                {
